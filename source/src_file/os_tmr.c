...
OS_STATIC_PTR_TYPE_A ptr_OS_TmrLock = OS_TmrLock;
OS_STATIC_PTR_TYPE_A ptr_OS_TmrUnlock = OS_TmrUnlock;
OS_STATIC_PTR_TYPE_A ptr_OS_TmrCondCreate = OS_TmrCondCreate;
OS_STATIC_PTR_TYPE_B ptr_OS_TmrCondWait = OS_TmrCondWait;
OS_STATIC_PTR_TYPE_A ptr_OS_TmrCondSignal = OS_TmrCondSignal;
#endif
/*
************************************************************************************************************************
*                                                   CREATE A TIMER
                    ...
                    OS_TICK   time)
{
    ...
    if (p_tmr1 == (OS_TMR *)0) {
        if (p_tmr2 == (OS_TMR *)0) {                            /* Remove the ONLY entry in the list?                   */
            ...
            if ((time - OSTmrTaskTickBase) > 0u) {      /* Reload if new head has a different delay    */
                OSTmrTaskTickBase   = time;
                OS_TmrCondSignal();
            }
