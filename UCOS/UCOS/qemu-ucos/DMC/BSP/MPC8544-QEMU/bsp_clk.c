...
#define  BSP_MC_MODE_TIMEOUT_VAL                            0xFFFFu
/*
*********************************************************************************************************
...
*/
    ...
    extern void CPU_TBU_Set(CPU_INT32U cnt);  
...
{
    /* 选择DEC(Decrimiter timer) */
    CPU_INT32U  hid0;
    CPU_INT32U  tcr;
    
    hid0    =  CPU_HID0_Get();  // 读spr
    hid0   |=  0x00004000;      // 设置hid0
    CPU_HID0_Set(hid0);
...
*********************************************************************************************************
*                                            BSP_CPU_ClkFreq()
*描述：返还CPU频率, 取自uboot中的get_cpu_freq(void)
*********************************************************************************************************
*/

CPU_INT32U  BSP_CPU_ClkFreq (void) {
  
  return ((CPU_INT32U)400000000UL);
}

/*
*********************************************************************************************************
*                                          BSP_Tick_Init()
*描述：Tick初始化,设置FIT(fixed-interval timer)间隔
*********************************************************************************************************
*/
...
void  BSP_Tick_Init (void) {
    ...
    cpu_clk_freq = BSP_CPU_ClkFreq(); // 获取CPU频率
    
#if (OS_VERSION >= 30000u)
    cnts  = cpu_clk_freq / (CPU_INT32U)OS_CFG_TICK_RATE_HZ; // OS_CFG_TICK_RATE_HZ设置为10000
#else
    cnts  = cpu_clk_freq / (CPU_INT32U)OS_CFG_TICK_RATE_HZ;
#endif
    decrementer_count = (unsigned int)cnts;

    CPU_DECAR_Set(cnts);        // 设置DECAR寄存器
    CPU_DEC_Set(cnts);          // 配置DEC寄存器

    CPU_TBL_Set(0);             // 配置TBL TBU
    CPU_TBU_Set(0);
